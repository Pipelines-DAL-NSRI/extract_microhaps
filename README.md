...
```--separate```: Separate output by microhaplotypes. Use flag to set to True. Default is False. Files will be in VCF format.
...
# Usage
```
python extract_by_pos.py --vcf input_file.vcf --markers list_of_microhaplotypes.csv --marker_sep ; --col_names Name,Chrom,Pos,ID --separate --outfile merged_files
```
